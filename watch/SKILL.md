...
| `3` | No Whisper API key | **Informational only.** Whisper is opt-in; the script works fine for any video with native captions. Do NOT proactively prompt the user about a key — only mention it if/when a video comes back without captions and the user wants transcription. |
| `4` | Both missing | Run installer (it auto-installs deps and scaffolds the `.env`). Same rule as `3` for the key — don't push the user on it unless transcription is needed. |
...
**API keys are opt-in — do not ask preemptively.** Whisper transcription is only invoked when the user explicitly passes `--whisper groq` or `--whisper openai`. Most YouTube/Vimeo/etc. videos have native captions, so the keys are not needed for default operation. Only when (a) a video comes back without captions AND (b) the user wants the audio transcribed should you walk them through adding a key (write it into `~/.config/watch/.env`, setting `GROQ_API_KEY=...` or `OPENAI_API_KEY=...`).
...
- `--whisper groq|openai` — **opt in** to Whisper transcription. Off by default. Pass this only when (a) a prior run came back without captions and (b) the user has confirmed they want the audio transcribed via API. Whisper calls cost money on the user's account.
...
1. **Native captions (free, default).** yt-dlp pulls manual or auto-generated subtitles from the source platform if available. This is what you get on every default `/watch` run.
2. **Whisper API (opt-in only).** If no captions are present, the report surfaces a "no captions found" callout and stops. **The script will not silently fall back to Whisper** — that would charge the user's API account without their consent. To transcribe, the user must explicitly re-run with `--whisper groq` or `--whisper openai`. The script then extracts audio (`ffmpeg -vn -ac 1 -ar 16000 -b:a 64k`, ~0.5 MB/min) and uploads it to:
   - **Groq** — `whisper-large-v3`. Cheaper, faster. Needs `GROQ_API_KEY` in `~/.config/watch/.env`. Get a key at console.groq.com/keys.
   - **OpenAI** — `whisper-1`. Needs `OPENAI_API_KEY` (also picked up from the process env). Get a key at platform.openai.com/api-keys.

**When a /watch run reports "no captions found":** ask the user via `AskUserQuestion` whether they want to transcribe via Whisper (cost: a fraction of a cent per minute), and which backend, before re-running. Default to "frames-only" if they decline.
...
- **No captions found callout in the report** → the source had no subtitle track and `--whisper` was not used (default). Tell the user plainly that the transcript is empty, and ask via `AskUserQuestion` whether they want to re-run with `--whisper groq` or `--whisper openai` to transcribe the audio (with a brief cost note), or proceed frames-only.
- **`--whisper` set but key missing** → the report's callout explains. Walk the user through adding the appropriate key to `~/.config/watch/.env`, then re-run.
...
- Does not upload the video itself to any API — only the extracted audio goes out, and only when the user has explicitly opted in by passing `--whisper groq` or `--whisper openai`. Default runs never call any API.
