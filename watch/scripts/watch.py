...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Opt in to Whisper transcription when no captions are available. "
            "Off by default — without this flag, the script reports 'no captions' "
            "and lets the caller decide whether to re-run with transcription."
        ),
    )
    args = ap.parse_args()
    ...
    if not transcript_segments and args.whisper:
        ...
        if backend and api_key:
            try:
                ...
                whisper_status = f"Whisper transcription failed: {exc}"
                print(f"[watch] {whisper_status}", file=sys.stderr)
        else:
            setup_py = SCRIPT_DIR / "setup.py"
            whisper_status = (
                f"`--whisper {args.whisper}` was requested but the matching API key is missing. "
                f"Add it to `~/.config/watch/.env` (run `python3 {setup_py}` to scaffold), then re-run."
            )
            print(f"[watch] {whisper_status}", file=sys.stderr)
    ...
    if transcript_text:
        ...
        print("> **No captions found for this video.** Frames above are the only signal — the audio has not been transcribed.")
        print(">")
        if whisper_status:
            print(f"> {whisper_status}")
        else:
            print("> Whisper transcription is opt-in. To transcribe the audio via API, re-run with one of:")
            print(">")
            print("> - `--whisper groq` (preferred — cheaper and faster; needs `GROQ_API_KEY`)")
            print("> - `--whisper openai` (needs `OPENAI_API_KEY`)")
            print(">")
            print("> Ask the user before re-running — Whisper calls cost money on their account.")
