---
name: storm-research
description: Use when someone asks to run Storm Research, use the storm-research skill, run the STORM method on a topic, says "storm research this" / "storm report on X" / "give me a STORM briefing on X", or wants a multi-perspective, citation-verified HTML research briefing on a topic. Best for topics where multiple viewpoints and fact-checked claims matter; overkill for a simple factual lookup.
...
Wait for ALL five lens agents to return before proceeding — if they run as background agents, keep collecting results as they arrive; never move on from partial returns. Then post a 2-3 line note in chat: which way they converge, and the sharpest disagreement. Keep raw briefs out of chat (the agents already returned them).
...
The report must be structurally identical to the template every time. Do NOT write HTML from scratch or from memory:

1. **Copy, don't rebuild.** `cp` the `report-template.html` that sits in this skill's folder to `storm-reports/{topic-slug}-briefing.html` (relative to the current working directory; create the folder first). Then edit that copy in place, section by section.
2. **Fill every `{{TOKEN}}`.** Each token carries its own instruction — follow it. Delete the instructional HTML comments as you fill each section. Duplicate only the blocks marked `REPEAT` (findings ×5, action items ×3-6, one reference row per citation). Never touch the `<style>` block, never rename classes, never add or remove sections. The only allowed class choices are the documented variants: `rel` = `high | medhigh | medium | low`, `vstat` = `ok | part | weak`.
3. Section mapping from the phases:
...
4. **Consistency gate (mandatory):** before Phase 4 ends, run `grep -c '{{' storm-reports/{topic-slug}-briefing.html` — the count MUST be 0. If any tokens remain, fill them and re-check. A report with leftover `{{tokens}}` or a modified `<style>` block is not a valid Storm Research report.
...
**4b. Verify every citation (parallel agents).** Spawn `general-purpose` agents in one message, one per distinct citation cluster (group related claims; ~4-6 agents). Wait for every verifier to return before applying corrections. Each agent prompt:
...
- Re-run the consistency gate from Phase 3 step 4 (zero `{{` tokens, no leftover instructional comments).
...
- **Design.** Clean white and professional (Montserrat / Roboto Mono, blue accent). The template CSS ships verbatim in every report — copying the file (Phase 3 step 1) is what guarantees this. Do not swap in a different visual style.
