---
name: roast
description: Use when someone asks to roast an idea, pressure-test or stress-test an idea, validate a business idea, "convene the council", get a brutal second opinion before building something, or says "/roast".
...
Spin up all five agents in parallel — one Agent tool call each (`subagent_type: general-purpose`), all five in a single message so they run concurrently. Paste the same brief into each, then give each its persona mandate below.
...
Wait until ALL five council members have returned before judging — if they run as background agents, keep collecting their results as they arrive and never deliver a verdict from partial returns. Once all five return, YOU act as the Judge. Read every council member's findings, weigh them, and synthesize one decisive verdict. Do not just average the scores. Name the real tension between the personas and resolve it.
